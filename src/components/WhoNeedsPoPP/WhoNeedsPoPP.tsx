...
export default function WhoNeedsPoPP() {
  ...
  const secondaryStakeholders = [
    { icon: <GraduationCap className="w-5 h-5 text-blue-400" />, group: "Academic Researchers", desc: "Study civic engagement patterns and governance effectiveness" },
    { icon: <Landmark className="w-5 h-5 text-blue-400" />, group: "Policy Makers", desc: "Data-driven policy formulation and impact assessment" },
    { icon: <Globe2 className="w-5 h-5 text-blue-400" />, group: "International Organizations", desc: "Cross-border transparency and human rights monitoring" },
    { icon: <Briefcase className="w-5 h-5 text-blue-400" />, group: "Private Sector", desc: "ESG compliance, stakeholder engagement, risk management" },
    { icon: <User className="w-5 h-5 text-blue-400" />, group: "Individual Citizens", desc: "Personal issue reporting and community participation" },
    { icon: <Code2 className="w-5 h-5 text-blue-400" />, group: "Technology Developers", desc: "Build applications and integrations on PoPP infrastructure" },
  ];
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="text-4xl lg:text-5xl font-extrabold text-white mb-4 tracking-tight">
            Who Needs <span className="text-cyan-400">PoPP?</span>
          </h2>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-16">
          {integrations.map((item) => (
            <div
              key={item.label}
              className="flex items-center gap-2 px-4 py-2 bg-white/[0.04] border border-white/[0.08] rounded-full text-sm text-gray-200"
            >
              <span>{item.icon}</span>
              {item.label}
            </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-300">
                        <span className="text-emerald-400">Benefits:</span> {user.benefits}
                      </p>
                    </div>
                  </div>
                </motion.div>
